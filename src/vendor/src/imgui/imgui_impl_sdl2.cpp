...
static void ImGui_ImplSDL2_YuriTea_UpdateKeyModifiers(uint32 key_mods) {
  ImGuiIO &io = ImGui::GetIO();
  io.AddKeyEvent(ImGuiMod_Ctrl, (key_mods & KMOD_CTRL) != 0);
  io.AddKeyEvent(ImGuiMod_Shift, (key_mods & KMOD_SHIFT) != 0);
  io.AddKeyEvent(ImGuiMod_Alt, (key_mods & KMOD_ALT) != 0);
  io.AddKeyEvent(ImGuiMod_Super, (key_mods & KMOD_GUI) != 0);
}
...
using namespace YuriTea;

bool ImGui_ImplSDL2_YuriTea_ProcessEvent(const YuriTea::Event* event){
  ...
  auto category = event-> GetCategoryFlags();
  constexpr const uint32 MouseMove = EventCategory::EventCategoryMouse | EventCategory::EventCategoryInput;
  constexpr const uint32 MouseButton = EventCategory::EventCategoryMouseButton | EventCategory::EventCategoryInput;
  constexpr const uint32 Keyboard = EventCategory::EventCategoryKeyboard | EventCategory::EventCategoryInput;
  ...
  case MouseMove: {
      switch((uint32)type){
        case (uint32)EventType::MouseMoved: {
          auto mouseEvent
              = static_cast<const MouseMovedEvent*>(event);
          ImVec2 mouse_pos(mouseEvent->GetPosition());
          io.AddMouseSourceEvent(mouseEvent->IsMouseMotion()
                               ? ImGuiMouseSource_Mouse
                               : ImGuiMouseSource_TouchScreen );
          io.AddMousePosEvent(mouse_pos.x, mouse_pos.y);
          return true;
        }
        case (uint32)EventType::MouseScrolled:{
          auto mouseEvent
              = static_cast<const MouseScrolledEvent*>(event);
          #if SDL_VERSION_ATLEAST(2, 0, 18) // If this fails to compile on Emscripten:
          // update to latest Emscripten!
          auto [wheel_x, wheel_y] = mouseEvent->GetOffset();
          #else
          float wheel_x = -(float)event->wheel.x;
          float wheel_y = (float)event->wheel.y;
          #endif
  ...
  case MouseButton: {
    switch ((uint32)type){
      case (uint32)EventType::MouseButtonPressed:
      case (uint32)EventType::MouseButtonReleased: {
        auto keyEvent = static_cast<const MouseButtonReleasedEvent*>(event);
        int32 mouse_button = -1;
        switch((uint16)keyEvent->GetButton()){
          case (uint16)MouseCode::BUTTON_LEFT:{
            mouse_button = 0; break;}
          case (uint16)MouseCode::BUTTON_RIGHT:{
            mouse_button = 1; break;}
          case (uint16)MouseCode::BUTTON_MIDDLE:{
            mouse_button = 2; break;}
          case (uint16)MouseCode::BUTTON_X1:{
            mouse_button = 3; break;}
          case (uint16)MouseCode::BUTTON_X2:{
            mouse_button = 4; break;}
          default: break;
        }
        io.AddMouseSourceEvent(keyEvent->IsMouseMotion()
                         ? ImGuiMouseSource_Mouse 
                         : ImGuiMouseSource_TouchScreen);
        io.AddMouseButtonEvent(mouse_button, (keyEvent->GetEventType() == EventType::MouseButtonPressed));
        io.AddMousePosEvent(keyEvent->GetPosition().x, keyEvent->GetPosition().y);
        bd->MouseButtonsDown = (keyEvent->GetEventType() == EventType::MouseButtonPressed)
  ...
  case TextInput: {
      if ((Uint32)type == (uint32)EventType::TextInput){
        auto textEvent = static_cast<const TextInputEvent*>(event);
        io.AddInputCharactersUTF8(textEvent->GetText().c_str());
        return true;}
      }
  case Keyboard: {
      if ((uint32)type == (Uint32)EventType::KeyPressed){
      auto keyEvent = static_cast<const KeyPressedEvent*>(event);
      ImGui_ImplSDL2_YuriTea_UpdateKeyModifiers(keyEvent->GetMod());
      ImGuiKey key = ImGui_ImplSDL2_KeycodeToImGuiKey(keyEvent->GetKeyCode());
      io.AddKeyEvent(key, true);
      auto scancode = SDL_GetScancodeFromKey(keyEvent->GetKeyCode());
      ...
      auto keyEvent = static_cast<const KeyReleasedEvent*>(event);
      ImGui_ImplSDL2_YuriTea_UpdateKeyModifiers(keyEvent->GetMod());
      ImGuiKey key = ImGui_ImplSDL2_KeycodeToImGuiKey(keyEvent->GetKeyCode());
  ...
  case Window: {
    if (type == EventType::WindowEnter) {
      ...
      bd->MouseWindowID = windowEvent->GetWindowID();
      bd->MouseLastLeaveFrame = 0;
    }
    if (type == EventType::WindowLeave){
      bd->MouseLastLeaveFrame = ImGui::GetFrameCount() + 1;
    }
    if (type == EventType::WindowFocus)
      io.AddFocusEvent(true);
    else if (type == EventType::WindowLostFocus)
      io.AddFocusEvent(false);
    return true;
  }
  case Gamepad:{
      switch ((uint32)type){
        case (uint32)EventType::ControllerDeviceAdded: 
        case (uint32)EventType::ControllerDeviceRemoved: {
          bd->WantUpdateGamepadsList = true;
          return true;
        }
      }
